CREATE OR REPLACE FUNCTION public.is_integrante_avaliado_por_dr(_integrante_id uuid)
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  SELECT EXISTS (
    ...
    WHERE ip.id = _integrante_id
      AND (
        upper(coalesce(ip.grau, '')) = 'V'
        OR lower(coalesce(ip.cargo_grau_texto, '')) LIKE '%diretor%divis%'
      )
  )
$$;
