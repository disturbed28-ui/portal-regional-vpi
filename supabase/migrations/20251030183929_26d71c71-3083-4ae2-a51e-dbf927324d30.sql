-- =====================================================
-- MIGRATION: Normalizar estrutura organizacional
-- =====================================================
...
-- Inserir Comando V
INSERT INTO public.comandos (nome) VALUES ('Comando V');

-- Guardar o ID do Comando V
DO $$
DECLARE
  cmd_id UUID;
...
BEGIN
  ...
  ON CONFLICT DO NOTHING
  RETURNING id INTO regional_vp1_id;
  
  IF regional_vp1_id IS NULL THEN
    SELECT id INTO regional_vp1_id FROM public.regionais WHERE nome = 'Vale do Paraiba I - SP';
  END IF;

  INSERT INTO public.regionais (comando_id, nome) 
  VALUES (cmd_id, 'Vale do Paraiba II - SP')
  ON CONFLICT DO NOTHING
  RETURNING id INTO regional_vp2_id;
  
  IF regional_vp2_id IS NULL THEN
    SELECT id INTO regional_vp2_id FROM public.regionais WHERE nome = 'Vale do Paraiba II - SP';
  END IF;

  INSERT INTO public.regionais (comando_id, nome) 
  VALUES (cmd_id, 'Litoral Norte - SP')
  ON CONFLICT DO NOTHING
  RETURNING id INTO regional_ln_id;
  
  IF regional_ln_id IS NULL THEN
    SELECT id INTO regional_ln_id FROM public.regionais WHERE nome = 'Litoral Norte - SP';
  END IF;
