...
import { useHistoricoEstagio } from "@/hooks/useHistoricoEstagio";
import { DivisaoEstagioCard } from "./DivisaoEstagioCard";
import { ReadOnlyBanner } from "@/components/ui/read-only-banner";

interface HistoricoEstagioProps {
  userId?: string;
  readOnly?: boolean;
}

export function HistoricoEstagio({ userId, readOnly = false }: HistoricoEstagioProps) {
  const {
    estagiosPorDivisao,
    divisoesDisponiveis,
    loading,
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Lista de estágios agrupados por divisão */}
      {estagiosPorDivisao.length === 0 ? (
        <Card className="border-border/50">
          <CardContent className="flex flex-col items-center justify-center py-12 text-center">
            <FileText className="h-12 w-12 text-muted-foreground/50 mb-4" />
            <h3 className="text-lg font-medium text-foreground mb-2">Nenhum registro encontrado</h3>
            <p className="text-sm text-muted-foreground max-w-xs">
              Não há estágios aprovados ou concluídos que correspondam aos filtros selecionados.
            </p>
          </CardContent>
        </Card>
      ) : (
        <div className="space-y-3">
          {estagiosPorDivisao.map((divisao) => (
            <DivisaoEstagioCard key={divisao.divisao_id} divisao={divisao} />
          ))}
        </div>
      )}
    </div>
